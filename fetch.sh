...
for domain in $DOMAINS; do
    ...
    for subnet in $SUBNETS; do
        case $subnet in
        *":"*) 
            dns=$(awk 'BEGIN{srand();} {print rand(), $0;}' dns6.txt | sort -k1,1n | head -n 1 | awk '{print $2}')
            dig $domain @$dns -t AAAA +subnet=$subnet +short >> $domain/ipv6.txt
        ...
        *)
            ...
            dig $domain @$dns -t A +subnet=$subnet +short >> $domain/ipv4.txt
